...
% estado_inicial(+Palabra, +Intentos, -Estado)
...
% Juega hasta terminar la partida
jugar(Estado) :-
...
% jugar_turno(+EstadoAnterior, -EstadoFinal, -Resultado)
jugar_turno(EstadoAnterior, EstadoFinal, Resultado) :-
    EstadoAnterior = estado(Palabra, Letras, Intentos),
    mostrar_palabra(Palabra, Letras, Visual),
    format("Palabra: ~w~n", [Visual]),
    format("Intentos restantes: ~w~n", [Intentos]),
    write("Ingresa una letra: "), flush_output(current_output),
    leer_letra(Letra),
    ( string_length(Letra, 1) ->
        adivinar_letra(Letra, EstadoAnterior, EstadoNuevo, R),
        (   R = repetida ->
            writeln("Letra ya usada\n"),
